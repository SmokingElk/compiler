...
Operand mapperLessEqual (BinCodeGen gen, List operands, Map typeTable) {
    ...
    instr = i_BCON(CMP_LE, shiftInstrAddr(gen->instrWritten, 3));
    writeInstruction(gen, instr);
    ...
    freeReg(gen, valueLocOperand(operand2));
